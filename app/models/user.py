...
class User:
    ...
    def fetch_by_password(cls, password):
        ...
        cursor.execute(
            "SELECT * FROM users WHERE password = %(password)s", {'password': password})
    ...
    @classmethod
    def approve_disaprove_status(cls, rId, request):
        connection = psycopg2.connect(
            "dbname='tracker_api' user='postgres' host='localhost' password='15december' port ='5432'")
        cursor = connection.cursor()
        cursor.execute("UPDATE requests SET status=%s WHERE id=%s",
                       (request['status'], rId))
    ...
    @classmethod
    def resolve_status(cls, rId, request):
        connection = psycopg2.connect(
            "dbname='tracker_api' user='postgres' host='localhost' password='15december' port ='5432'")
        cursor = connection.cursor()
        cursor.execute("UPDATE requests SET status=%s WHERE id=%s",
